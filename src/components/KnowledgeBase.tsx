...
export function KnowledgeBase() {
  ...
  useEffect(() => {
    ...
    const loadCozeSDK = () => {
      return new Promise<void>((resolve, reject) => {
        if (document.querySelector('script[src*="chat-app-sdk"]')) {
          resolve();
          return;
        }
        ...
        script.onerror = () => reject(new Error("Coze SDK Load Failed"));
    ...
    const initCoze = async () => {
      try {
        ...
        if (!CozeWebSDK || !containerRef.current) return;
        ...
        const client = new CozeWebSDK.WebChatClient({
          ...
          ui: {
            ...
            chatBot: {
                el: containerRef.current, // 关键配置：指定渲染容器
                width: '100%', 
                isNeedQuote: true, // 启用消息追问功能
            },
            conversations: {
              isNeed: true,
            },
      ...
      nickname: 'User',
        ...
        console.error("Coze init failed:", error);
      }
    };
  ...
  return (
    <div className="relative h-full w-full bg-background overflow-hidden" ref={containerRef}>
      {/* 
         保留 CSS 作为双重保险，特别是针对关闭按钮的隐藏。
         虽然配置了 isNeedClose: false，但 CSS 隐藏总是最稳妥的。
      */}
      <style>{`
        /* 确保容器内的 SDK 元素没有多余的 margin/padding */
        .coze-webchat,
